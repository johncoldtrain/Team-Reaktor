...
class UserFriendshipTest < ActiveSupport::TestCase
  ...
  context "a new instance" do
    setup do
      @user_friendship = UserFriendship.new user: users(:alex), friend: users(:mike)
    end
  ...
  context "#send_request_email" do
    ...
    setup do
      @user_friendship = UserFriendship.create user: users(:alex), friend: users(:mike)
    end
  ...
  context "#mutual_friendship" do
    setup do
      UserFriendship.request users(:alex), users(:perro)
      @friendship1 = users(:alex).user_friendships.where(friend_id: users(:perro).id).first
      @friendship2 = users(:perro).user_friendships.where(friend_id: users(:alex).id).first
    end
  ...
  end # --- #accept_mutual_friendship!


  context "#accept!" do
    setup do
      @user_friendship = UserFriendship.request users(:alex), users(:mike)
    end
  ...
  context ".request" do
    ...
    should "create two user friendships" do
      assert_difference 'UserFriendship.count', 2 do
        UserFriendship.request(users(:alex), users(:mike))
      end
    end
  ...
  context "#block!" do
    setup do
      @user_friendship = UserFriendship.request users(:alex), users(:perro)
    end
    ...
    should "not allow new requests once blocked" do
      @user_friendship.block!
      uf = UserFriendship.request users(:alex), users(:perro)
      assert !uf.save
    end
